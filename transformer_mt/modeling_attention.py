...
class MultiHeadAttention(nn.Module):
    def __init__(self, input_size, hidden, num_heads, causal=False):
        ...
        if hidden % num_heads:
            raise ValueError(
                f"hidden should be divisible by num_heads, "
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        k = (
            k.transpose(1, 2)
            .reshape(bs * self.num_heads, self.head_size, -1)
            .transpose(1, 2)
            .contiguous()
        )  # [batch * num_heads, seq, hidden / num_heads]
        q = (
            q.transpose(1, 2)
            .reshape(bs * self.num_heads, self.head_size, -1)
            .transpose(1, 2)
        ...
        )  # [batch * num_heads, attending_seq, attended_seq]
        ...
        if self.causal:
            causal_mask = torch.triu(
                torch.ones(
                    attending_seq, attended_seq, dtype=torch.bool, device=scores.device
                ),
                diagonal=1,
            )
