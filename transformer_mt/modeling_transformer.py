...
class TransformerDecoderLayer(nn.Module):
    def __init__(self, hidden, num_heads, fcn_hidden, dropout=0.0):
        ...
        self.self_attention = MultiHeadAttention(
            input_size=hidden, hidden=hidden, num_heads=num_heads, causal=False
        )
        self.cross_attention = MultiHeadAttention(
            input_size=hidden, hidden=hidden, num_heads=num_heads, causal=True
        )
        ...
        self.fcn = nn.Sequential(
            nn.Linear(hidden, fcn_hidden), nn.ReLU(), nn.Linear(fcn_hidden, hidden)
        )
        self.fcn_layer_norm = nn.LayerNorm(hidden)
        self.dropout = nn.Dropout(dropout)
        # YOUR CODE ENDS HERE
    ...
    def forward(
        self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None
    ):
        ...
        self_att = self.self_attention(
            residual, encoder_hidden_states, key_padding_mask=key_padding_mask
        )
...
class TransfomerEncoderDecoderModel(nn.Module):
    ...
    def forward(
        ...
        if input_ids is None and encoder_hidden_states is None:
            raise ValueError(
                "You should provide either input_ids or encoder_hidden_states"
            )
        ...
        logits = self._decode(
            encoder_hidden_states, decoder_input_ids, key_padding_mask
        )
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        for layer in self.decoder_layers:
            trg_embedding = layer(
                trg_embedding, encoder_hidden_states, key_padding_mask=key_padding_mask
            )
    ...
    def generate(
        ...
        *,
        ...
        for i in range(input_ids.size(0)):
            ...
            _key_padding_mask = (
                key_padding_mask[i].unsqueeze(0)
    ...
    def _generate_greedy(
        ...
        *,
        ...
        eos_flags = torch.zeros(
            (input_ids.shape[0],), dtype=torch.uint8, device=input_ids.device
        )
    ...
    def _generate_beam_search(
        ...
        *,
        ...
        assert (
            len(input_ids) == 1
        ), "Beam search is only supported for a single input sequence"
        encoder_hidden_states = self._encode(input_ids, key_padding_mask)
        device = input_ids.device
        ...
        for _ in range(max_length):
            ...
            hypotheses_tensor = torch.tensor(
                hypotheses, dtype=torch.int64, device=device
            )
            logits = self._decode(
                expanded_encoder_hidden_states, hypotheses_tensor, key_padding_mask
            )
            ...
            prev_hyp_ids = torch.div(
                top_new_hyp_pos, self.tgt_vocab_size, rounding_mode="floor"
            )
            ...
            for prev_hyp_id, hyp_token_id, cand_new_hyp_score in zip(
                prev_hyp_ids, token_ids, top_new_hyp_scores
            ):
    ...
    def from_pretrained(cls, save_path, map_location=None):
        ...
        state_dict = torch.load(
            os.path.join(save_path, "model.pt"), map_location=map_location
        )
