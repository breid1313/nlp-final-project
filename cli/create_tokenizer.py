...
def main():
    ...
    logger.info(
        f"Building tokenizer for the source language (might take a couple of minutes)"
    )
    ...
    source_tokenizer_trainer = BpeTrainer(
        special_tokens=["[UNK]", "[PAD]", "[BOS]", "[EOS]"], vocab_size=args.vocab_size
    )

    source_tokenizer.pre_tokenizer = Whitespace()

    source_iterator = (
        item["translation"][args.source_lang] for item in raw_datasets["train"]
    )
    ...
    source_tokenizer.save_pretrained(
        os.path.join(args.save_dir, f"{args.source_lang}_tokenizer")
    )

    # YOUR CODE ENDS HERE

    logger.info(
        f"Building tokenizer for the target language (might take a couple of minutes)"
    )
    ...
    target_iterator = (
        item["translation"][args.target_lang] for item in raw_datasets["train"]
    )
    ...
    target_tokenizer.save_pretrained(
        os.path.join(args.save_dir, f"{args.target_lang}_tokenizer")
    )
