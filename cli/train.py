...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="en-de",
        help=(
            "Many datasets in Huggingface Dataset repository have multiple versions or configs. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="The number of processes to use for the preferenceing.",
    )
    parser.add_argument(
        "--overwrite_cache",
        type=bool,
...
def collation_function_for_seq2seq(batch, source_pad_token_id, target_pad_token_id):
    ...
    collated_batch["encoder_padding_mask"] = (
        collated_batch["input_ids"] == source_pad_token_id
    )
...
def main():
    ...
    for index in random.sample(range(len(train_dataset)), 2):
        ...
        logger.info(
            f"Decoded input_ids: {source_tokenizer.decode(train_dataset[index]['input_ids'])}"
        )
        logger.info(
            f"Decoded labels: {target_tokenizer.decode(train_dataset[index]['labels'])}"
        )
        logger.info("\n")
    ...
    logger.info(
        "Look at the data that we input into the model, check that it looks like what we expect."
    )
    for index in random.sample(range(len(batch)), 2):
        logger.info(
            f"Decoded input_ids: {source_tokenizer.decode(batch['input_ids'][index])}"
        )
        logger.info(
            f"Decoded labels: {target_tokenizer.decode(batch['labels'][index])}"
        )
        logger.info("\n")
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            if global_step % args.logging_steps == 0:
                ...
                accuracy = (predictions == labels).masked_select(
                    label_nonpad_mask
            ...
            if (
                ...
                random_index = random.randint(0, len(eval_input_ids) - 1)
                logger.info(
                    f"Input sentence: {source_tokenizer.decode(eval_input_ids[random_index], skip_special_tokens=True)}"
                )
